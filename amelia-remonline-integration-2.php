...
class Amelia_Remonline_Integration
{
    ...
    public function __construct()
    {
        ...
        if (!wp_next_scheduled('remonline_sync_appointments')) {
            wp_schedule_event(time(), 'hourly', 'remonline_sync_appointments');
        }
...
?>
    ...
    public function handle_appointment_created($appointmentId, $appointmentData)
    {
        ...
        try {
            ...
            // Создаем клиента в Remonline
            $this->log("Створюємо клієнта у Remonline: {$appointment_details['customer']['firstName']} {$appointment_details['customer']['lastName']}", 'debug');
            $client_id = $this->create_remonline_client($appointment_details['customer'], $api_token);

            if (!$client_id) {
                $this->log("Не вдалося створити клієнта у Remonline", 'error');
                return;
            }
            ...
            $this->log("Успішно створено клієнта (ID: $client_id) та замовлення (ID: $order_id) у Remonline", 'info');
